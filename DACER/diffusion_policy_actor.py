...
def _precompute_diffusion_schedule(
    ...
    return {
        ...
        "alphas_cumprod_prev": alphas_cumprod_prev,  # <-- 现在包含了这个关键的buffer
    }
...
class DiffusionPolicyActor(BasePolicy):
    ...
    def _sample_from_noise(
        ...
        for t_step in reversed(range(1, self.T + 1)):
            ...
            # 获取当前时间步的alpha和beta值
            alphas_t = self.alphas_cumprod.gather(0, t)
            alphas_t_prev = self.alphas_cumprod_prev.gather(0, t)
            betas_t = self.betas.gather(0, t)

            # 计算去噪后的动作均值
            pred_original_start = (
                action_t - torch.sqrt(1.0 - alphas_t).view(-1, 1) * predicted_noise
            ) / torch.sqrt(alphas_t).view(-1, 1)
            pred_original_start = torch.clamp(pred_original_start, -1.0, 1.0)

            posterior_mean = (
                betas_t.view(-1, 1)
                * torch.sqrt(alphas_t_prev).view(-1, 1)
                * pred_original_start
                + torch.sqrt(1.0 - betas_t).view(-1, 1)
                * (1.0 - alphas_t_prev).view(-1, 1)
                * action_t
            ) / (1.0 - alphas_t).view(-1, 1)
