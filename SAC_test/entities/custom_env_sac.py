...
class CustomEnv(gym.Env):
    ...
    def __init__(
        ...
        self.action_space = spaces.Box(
            low=np.array(
                [0, -np.pi, 0, 0], dtype=np.float32
            ),  # ue_id, angle, velocity_ratio, offloading_ratio
            high=np.array([self.ue_num - 1, np.pi, 1, 1], dtype=np.float32),
    ...
    # --- 4. 函数职责单一化：将庞大的 step 函数拆解 ---
    def _process_action(self, action: np.ndarray) -> Tuple[int, float, float, float]:
        """从原始动作向量中解码、取整和裁剪，返回清晰的动作分量。"""
        ue_id = int(np.round(action[0]))
        angle = float(action[1])
        velocity_ratio = float(action[2])
        offloading_ratio = float(action[3])

        # 确保动作在有效范围内
        ue_id = np.clip(ue_id, 0, self.ue_num - 1)
        velocity_ratio = np.clip(velocity_ratio, 0.0, 1.0)
        offloading_ratio = np.clip(offloading_ratio, 0.0, 1.0)

        return ue_id, angle, velocity_ratio, offloading_ratio
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        # 1. 解码动作
        ue_id, angle, velocity_ratio, offloading_ratio = self._process_action(action)
